...
public class Scala {
	...
	public static <A extends B, B> scala.collection.immutable.Seq<B> asScala(Collection<A> javaCollection) {
		final scala.collection.immutable.List<A> as = scala.jdk.javaapi.CollectionConverters.asScala(javaCollection)
				.toList();
	 ...
	 *
	 * <p>
	 * A PartialFunction is one that isn't defined for the whole of its domain. If
	 * the function isn't defined for a particular input parameter, it can throw
	 * <code>F.noMatch()</code>, and this will be translated into the semantics of a
	 ...
	 * </pre>
	 *
	 * The above code will convert a flow of String into a flow of Integer, dropping
	 * any strings that can't be parsed as integers.
	 *
